...
def getPage(pageNum):
    ...
    selector = html.fromstring(requests.get(baseUrl, headers=myheaders).content)
    print(selector.xpath('//ul[@id="pins"]/li/a/@href'))
    urls = []
...
# url是详情页链接
def getPiclink(url):
    sel = html.fromstring(requests.get(url, headers=myheaders).content)
    # 图片总数
    ...
    for i in range(int(total)):
        ...
        try:
            link = '{}/{}'.format(url, i+1)
            s = html.fromstring(requests.get(link, headers=myheaders).content)
            # 图片地址在src标签中
